...
const registerPortalClient = async (req, res) => {
  const dbClient = await getClient();
  
  try {
    const { email, password, firstName, lastName, tenantId, razonSocial, telefono, cuit } = req.body;
    const normalizedEmail = email.toLowerCase().trim();
    ...
    const tenantResult = await dbClient.query(
      'SELECT id, name, portal_enabled FROM tenants WHERE id = $1 AND is_active = true',
      [tenantId]
    );
    ...
    if (!tenant.portal_enabled) {
      ...
      return res.status(400).json({
        success: false,
        message: 'El portal de esta organización no está habilitado'
      });
    }
    ...
    // Crear usuario como cliente del tenant
    const userResult = await dbClient.query(
      `INSERT INTO users (
        email, password_hash, first_name, last_name,
        tenant_id, role, auth_provider,
        email_verification_token, email_verification_expires
      ) VALUES ($1, $2, $3, $4, $5, 'client', 'local', $6, $7)
      RETURNING id, email, first_name, last_name, role`,
      [normalizedEmail, passwordHash, firstName, lastName, tenantId, verificationToken, verificationExpires]
    );
    ...
    await dbClient.query(
      `INSERT INTO clientes (
        tenant_id, razon_social, nombre_fantasia, tipo_documento, numero_documento,
        email, telefono, es_cliente, user_id, pais
      ) VALUES ($1, $2, $3, $4, $5, $6, $7, true, $8, 'Argentina')`,
      [
        tenantId,
        clienteNombre,
        firstName,
    ...
    await query(
      `SELECT log_audit($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
      [
        tenantId, newUser.id, 'CLIENT_REGISTERED', 'users', newUser.id,
        null, JSON.stringify({ email: normalizedEmail }),
        req.ip, req.headers['user-agent']
      ]
    ).catch(err => console.error('Error en auditoría:', err));
    ...
    await dbClient.query('ROLLBACK');
    console.error('Error registrando cliente portal:', error);
    res.status(500).json({
      success: false,
