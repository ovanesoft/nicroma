...
// Campos que no deben ser sanitizados/verificados (contraseñas, base64, certificados, URLs, etc.)
const SKIP_FIELDS = [
  // Contraseñas
  'password', 'password_hash', 'currentPassword', 'newPassword', 'confirmPassword',
  // Archivos/Base64
  'logo', 'certificate', 'privateKey', 'certificatePassword', 'avatar',
  // URLs y campos que pueden tener caracteres especiales
  'website', 'companyWebsite', 'domain', 'url', 'callback_url', 'redirect_uri',
  // Direcciones y textos libres
  'address', 'companyAddress', 'domicilioFiscal', 'description', 'observaciones',
  'welcomeMessage', 'portalWelcomeMessage', 'message', 'content', 'body', 'html',
  // Colores (tienen #)
...
const attackDetection = (req, res, next) => {
  ...
  // Omitir verificación para rutas que manejan archivos/base64 o configuraciones
  if (req.path.includes('/logo') || 
      req.path.includes('/certificate') || 
      req.path.includes('/avatar') ||
      req.path.includes('/company') ||
      req.path.includes('/fiscal/config')) {
    return next();
  }
  ...
  // Verificar body (excluyendo campos sensibles)
  if (checkObject(req.body, SKIP_FIELDS)) {
